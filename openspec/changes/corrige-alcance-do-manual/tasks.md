...
- [x] 1.1 `apps/api/src/config.ts`: declarar a constante do **endereço canônico**
...
- [x] 1.2 `apps/api/src/config.ts`: trocar `optional('APP_MANUAL_URL', '')` por
...
- [x] 1.3 `apps/api/src/app.ts`: garantir que o override
...
- [x] 1.4 Confirmar que `packages/shared`, `apps/web` e os três arquivos de
...
- [x] 2.1 `infra/terraform/variables.tf`: corrigir a `description` de
...
- [x] 2.2 `infra/terraform/terraform.tfvars.example`: mesmo ajuste no comentário,
  mantendo a linha comentada (agora é de fato opcional).
- [x] 2.3 **Não** alterar `cloud_run.tf` e **não** exigir `terraform apply` — a
  ...
  Verificado: `cloud_run.tf` continua definindo `APP_MANUAL_URL = var.app_manual_url`
  incondicionalmente — sem alteração, como previsto.

## 3. Sandbox — reconciliação do `.env`

- [x] 3.1 `.claude/hooks/session-start.sh`: além de copiar `.env.example` quando
...
- [x] 3.2 Garantir as três não-ações exigidas pela spec: não sobrescrever valor
...
- [x] 3.3 Verificar idempotência: rodar o hook duas vezes seguidas não duplica
  chaves nem altera o arquivo na segunda execução.
  Verificado isoladamente (script de teste com `.env` local com valores
  diferentes do exemplo): primeira execução acrescenta só a chave ausente,
  preserva valores locais; segunda execução não altera o arquivo.
- [x] 3.4 Validar no sandbox atual que `APP_MANUAL_URL` chega ao `.env` existente
  — é o caso concreto que originou a change.
  Validado diretamente no `.env` deste sandbox: com a chave removida, a
  reconciliação a repõe com o valor do exemplo, sem tocar no resto do
  arquivo. (O hook completo não pôde ser exercitado ponta a ponta neste
  sandbox porque o binário `fake-gcs-server` não está instalado aqui —
  lacuna de provisionamento preexistente, sem relação com esta change; o
  trecho de reconciliação foi validado isoladamente com o conteúdo real de
  `.env`/`.env.example`.)

## 4. Testes

- [x] 4.1 Teste novo (API): o endereço canônico da aplicação é **igual** ao
  ...
  `apps/api/src/__tests__/config-manual-url.test.ts`.
- [x] 4.2 Testes novos da resolução, exercitada como função com entradas
  ...
  Os três primeiros casos em `config-manual-url.test.ts` (`resolveManualUrl`,
  função pura); o caso inválido já é coberto via `createApp` em
  `auth.test.ts` (a validação de esquema é responsabilidade de `app.ts`, não
  da resolução em si).
- [x] 4.3 `apps/api/src/__tests__/auth.test.ts`: o caso hoje chamado
...
- [x] 4.4 `apps/api/src/__tests__/auth.test.ts`: a asserção de contrato
...
- [x] 4.5 **Não** remover o teste de web "item ausente quando `manualUrl` está
...
- [x] 5.1 `.env.example`: ajustar o comentário de `APP_MANUAL_URL` para override
  opcional (o valor sugerido já é o canônico e pode permanecer).
- [x] 5.2 `docs/manual/docs/a-tela.md`: verificar a redação atual do item do
  manual; se ela condicionar a existência do acesso à configuração da
  implantação, corrigir — o acesso passa a existir sempre.
  Verificado: a redação atual ("Ele aparece para qualquer perfil, em
  qualquer tela...") já não condiciona a existência do item a nenhuma
  configuração — nenhuma mudança necessária.
- [x] 5.3 Rodar `npm run lint`, `npm run build`, `npm run test` e
  `npm run format:check` na raiz. Lembrar que `openspec/` e `docs/` estão fora do
  Prettier de propósito.
  Todos verdes: lint (3 workspaces), build (shared/api/web), test (243 API +
  115 web), format:check.
