...
- [x] 1.1 `packages/shared/src/auth.ts`: `PublicConfigResponse` ganha
...
- [x] 1.2 Recompilar: `npm run build --workspace packages/shared`. Sem isso,
  api e web continuam enxergando o DTO antigo (o pacote é consumido de
  `dist/`, não da fonte).

## 2. API — configuração e endpoint

- [x] 2.1 `apps/api/src/config.ts`: `appManualUrl: optional('APP_MANUAL_URL',
...
- [x] 2.2 Validação de esquema com falha no arranque: valor não vazio cujo
...
- [x] 2.3 `apps/api/src/routes/auth.ts`: `GET /auth/public-config` passa a
...
- [x] 2.4 Confirmar que nenhum prefixo de rota mudou — `api-prefixes.ts`,
...
- [x] 3.1 `apps/web/src/lib/schemas.ts`: `publicConfigResponseSchema` ganha
...
- [x] 3.2 `apps/web/src/auth/session-context.tsx`: `DEFAULT_PUBLIC_CONFIG`
...
- [x] 4.1 `apps/web/src/shell/AppShell.tsx`: bloco de rodapé com um segundo
  ...
  é vazia.
- [x] 4.2 Resolver a incógnita mecânica registrada em design.md D6: verificar
...
- [x] 4.3 O item abre em nova aba: `target="_blank"`,
...
- [x] 4.4 Rótulo "Manual do usuário" (fiel ao `site_name` do MkDocs) e ícone
  do conjunto já usado no shell. Confirmar que o tooltip do estado colapsado
  aparece e traz o rótulo completo.

## 5. Configuração de ambiente

- [x] 5.1 `.env.example`: `APP_MANUAL_URL`, junto de `APP_CLIENT_NAME`,
...
- [x] 5.2 Registrar que em produção o valor entra como variável de ambiente do
  Cloud Run — dado público, **fora** do Secret Manager, mesmo tratamento de
  `APP_CLIENT_NAME`. `APP_CLIENT_NAME` é gerido por Terraform
  (`variables.tf`/`cloud_run.tf`), então `APP_MANUAL_URL` seguiu exatamente o
  mesmo padrão: `var.app_manual_url` (default `""`) +
  `env { name = "APP_MANUAL_URL" }` em `cloud_run.tf` +
  `terraform.tfvars.example`. Nenhum prefixo de rota mudou — `locals.tf`
  (`api_proxy_prefixes`) permanece intocado.

## 6. Testes

- [x] 6.1 `apps/api/src/__tests__/auth.test.ts`: a asserção
...
- [x] 6.2 API: caso de arranque com `APP_MANUAL_URL` inválida (esquema não
  web) falhando com mensagem que nomeia a variável; e caso com valor vazio
  arrancando normalmente e respondendo `manualUrl: ''`.
- [x] 6.3 Web, teste novo no shell: item presente com `href`, `target` e `rel`
...
- [x] 6.4 Rodar a suíte completa dos dois workspaces — o contrato do
  `public-config` é lido por vários testes de web que montam a sessão.
  Fixtures ajustadas em `login.test.tsx`, `shell-identidade-visual.test.tsx` e
  `download-pasta.test.tsx` (mock de `GET /auth/public-config` sem
  `manualUrl` fazia o Zod rejeitar e cair no fallback). 239 testes da API e
  115 da web, todos verdes.

## 7. Documentação

- [x] 7.1 `docs/manual/docs/a-tela.md`: descrever o acesso ao manual no rodapé
...
- [x] 7.2 Conferir se `docs/manual/docs/index.md` ou `primeiro-acesso.md`
  merecem menção ao acesso. **Não** documentar o item como disponível antes do
  login: ele não existe na tela de login (design.md D7). Nenhuma das duas
  páginas descreve telas ou navegação pós-login em detalhe (isso já é
  trabalho de `a-tela.md`, referenciada em "Por onde continuar" de
  `index.md`) — nenhuma mudança necessária.
- [x] 7.3 Não formatar `docs/` nem `openspec/` com Prettier — estão fora do
  escopo por decisão (`.prettierignore`, já cobre os dois diretórios). Rodar
  `npm run format` apenas sobre o código alterado.

## 8. Fechamento

- [x] 8.1 `npm run lint`, `npm run build`, `npm run test` e
  `npm run format:check` limpos na raiz.
- [x] 8.2 `openspec validate acesso-ao-manual-no-shell --strict`. Precisou
  reverter o título de dois cenários do delta `identidade-visual` (renomeados
  na primeira versão) para os títulos do spec base — o `--strict` recusa um
  requisito MODIFIED que solte um título de cenário existente; o corpo dos
  dois cenários continua refletindo a allowlist de três valores.
- [x] 8.3 Conferência visual final no navegador, autenticado, nos dois estados
  do Sider e em pelo menos dois papéis (`collaborator` e `unit_admin`), para
  confirmar que o item não varia por papel. Verificado via Playwright contra
  `dev:api`+`dev:web` reais: expandido e colapsado (tooltip "Manual do
  usuário" em texto plano no colapsado), idêntico para `collaborator` e
  `unit_admin` apesar dos itens extras de administração, `href`/`target`/
  `rel`/`aria-label` corretos, e **ausente** na tela de login (design.md D7).
