...
- [x] 1.1 Antes de escrever a migração, verificar na base de dev e na de produção
...
- [x] 1.2 `apps/api/src/db/migrations/0014_*.sql`: bloco `DO $$` de detecção de
...
- [x] 1.3 `DROP` e recriação de `folders_unit_parent_name_uidx` com
  `NULLS NOT DISTINCT`, mantendo `WHERE deleted_at IS NULL` (Épico 6: pasta
  excluída não ocupa o nome).
- [x] 1.4 Ampliar o `CHECK` de `audit_events.action` com `'move'`, no mesmo padrão
  incremental de `0004` e `0008` (`DROP CONSTRAINT` + `ADD CONSTRAINT`).
- [x] 1.5 Nenhuma coluna nova, nenhum backfill, nenhuma tabela nova — logo, nenhuma
...
- [x] 2.1 `AuditAction.MOVE` em `audit.ts`.
- [x] 2.2 DTO de mover, com o destino sempre **presente** e admitindo `null`
...
- [x] 2.3 DTO de renomear pasta (`{ name: string }`) e resposta reusando
  `FolderResponse`.
- [x] 2.4 Códigos de erro identificáveis para as duas recusas específicas
  (conflito de nome e destino inválido por ciclo), para a SPA distinguir os avisos.
- [x] 2.5 `npm run build --workspace packages/shared`.

## 3. Resolução de alcance (`apps/api/src/lib/access.ts`)

- [x] 3.1 Nova função de alcance "**dono OU admin da unidade**, sem ramo de
...
- [x] 3.2 Reusar `isAdminOfUnit` para herdar a trava do bypass de `global_admin`
  (comparação explícita `resource.unit_id === ctx.unitId`). **Não** replicar a
  lógica de admin à mão.
- [x] 3.3 Comentário no arquivo explicando por que esta resolução **não** consulta
...
- [x] 4.1 Verificação de ciclo **antes** do `UPDATE`: recusar se o destino é a
  própria pasta ou pertence à sua subárvore, reusando a travessia recursiva já
  existente.
- [x] 4.2 Reverificação **depois** do `UPDATE` e antes do commit, na mesma
...
- [x] 4.3 Guarda de ciclo em `buildBreadcrumb` (`routes/folders.ts`): conjunto de
  visitados e teto de profundidade, terminando com erro tratado em vez de laço
  infinito.
- [x] 4.4 Guarda de ciclo em `collectSubtreeFolderIds`: `UNION` com ciclo detectado
  ou cláusula `CYCLE`, em vez do `UNION ALL` nu.
- [x] 4.5 Verificação de colisão de nome em pasta, com erro identificável
...
- [x] 4.6 **Remover** a leitura-remendo de raiz de `findOrCreateChild` /
...
- [x] 4.7 Corrigir o comentário de `buildBreadcrumb` que afirma que "todo ancestral
  já pertence ao mesmo dono" — já é falso desde o Épico 3 e esta mudança o torna
  rotineiro (design.md, Risks).

## 5. Rotas da API

- [x] 5.1 `POST /files/:id/move` em `routes/files.ts`: alcance sobre o arquivo
  **e** sobre o destino (seção 3), `UPDATE files SET folder_id = $1`, e nada mais —
  `object_path`, `size_bytes`, `owner_id` e `status` intocados.
- [x] 5.2 Auditar `move` **por arquivo movido**, no mesmo padrão de `recordAudit`
  já usado por `rename`/`replace`. Só no sucesso.
- [x] 5.3 `POST /folders/:id/move` em `routes/folders.ts`: alcance sobre a pasta
  **e** sobre o destino, verificação de ciclo (4.1/4.2), verificação de colisão
  ...
  (design.md D6).
- [x] 5.4 `PATCH /folders/:id`: alcance sobre a pasta, verificação de colisão,
  `UPDATE folders SET name = $1`. Espelhar a validação de nome do
  `PATCH /files/:id` (string não-vazia após `trim`). **Sem** escrita em
  `audit_events`.
- [x] 5.5 Destino nulo aceito nas três rotas como "raiz da unidade", sem exigir
  alcance sobre a raiz (design.md D1).
- [x] 5.6 Recusas fail-closed e **indistinguíveis** para inexistente / outra
...
- [x] 5.7 Confirmar que nenhum prefixo de rota de topo foi criado — `/files` e
...
- [x] 6.1 `MoverItemModal`: seletor de destino navegando nível a nível sobre
...
- [x] 6.2 Generalizar `RenameFileModal` para atender pasta também, ou extrair o
  modal comum — sem duplicar a validação de nome.
- [x] 6.3 Ações "Mover para..." e "Renomear" (pasta) no menu por linha do
  `ExplorerPage`, entrando no `GroupedActions` em tela estreita como as demais.
- [x] 6.4 Mutações em `queries.ts`, invalidando a listagem da pasta corrente ao
  sucesso.
- [x] 6.5 Avisos **distinguíveis** para as três recusas: permissão insuficiente,
  destino inválido por ciclo e conflito de nome.
- [x] 6.6 Não inferir permissão no cliente em nenhum ponto — oferecer a ação e
  deixar o `403` falar, como o resto do explorador já faz.

## 7. Manual do usuário (`docs/manual`)

- [x] 7.1 `docs/manual/docs/colaborador/renomear-e-excluir.md`: cobrir **mover**
  arquivo e pasta e **renomear pasta**, com os rótulos exatos exibidos na tela.
- [x] 7.2 Dizer com todas as letras que **mover para uma pasta compartilhada não
...
- [x] 7.3 Registrar as recusas que o usuário pode encontrar: nome já existente no
  destino e destino dentro da própria pasta.
- [x] 7.4 Avaliar se `referencia/tarefas-rapidas.md` merece a entrada "mover um
  arquivo"; ajustar o título da página em `mkdocs.yml` se o escopo dela deixar de
  ser descrito por "Renomear e excluir".
- [x] 7.5 Documentar apenas o que a tela entrega — nada de capacidade só de
  backend (regra dura da capability `documentacao-usuario`).

## 8. Testes

- [x] 8.1 **Ciclo direto**: mover pasta para si mesma ⇒ recusa, hierarquia intacta.
- [x] 8.2 **Ciclo em profundidade**: mover pasta para uma descendente a 3+ níveis
  ⇒ recusa.
- [x] 8.3 **Travessias resistentes a ciclo**: com uma linha em ciclo forçada
  diretamente no banco, `buildBreadcrumb` e `collectSubtreeFolderIds` terminam com
  erro tratado, sem pendurar o processo.
- [x] 8.4 **Colisão na raiz** — o caso que o índice não pegava antes: mover/renomear
  pasta para nome já ocupado na raiz ⇒ recusa. Este teste é a razão de ser da
  migração `0014`.
- [x] 8.5 Colisão sob pasta-pai e colisão que difere só por maiúsculas ⇒ recusa;
  homônima **na lixeira** ⇒ aceita.
- [x] 8.6 Arquivos homônimos na mesma pasta após mover ⇒ aceito (sem constraint).
- [x] 8.7 **Sem alcance**: não-dono com grant `rename` sobre o item e `upload`
  sobre o destino ⇒ `403` nas três rotas. Codifica o recorte de D2.
- [x] 8.8 **Isolamento**: `global_admin` de outra unidade ⇒ `403`, indistinguível
  de inexistente. Somado ao `isolamento-unidade.test.ts` (3.5); cobertura própria
  também em `mover-e-renomear.test.ts`. `rls-isolation.test.ts` testa a RLS bruta
  do Postgres, sem rota — não se aplica a `canReorganize`, que não muda policy.
- [x] 8.9 **Lixeira**: item na lixeira não é origem nem destino, nas três rotas.
- [x] 8.10 **Preservação**: após mover, `object_path` inalterado, `size_bytes` e
  espaço utilizado do dono inalterados, concessões sobre o item ainda vigentes e
  `audit_events` anteriores preservados.
- [x] 8.11 **Auditoria**: mover arquivo grava um evento `move`; mover/renomear
  pasta não gravam nada; `GET /files/:id/audit` continua devolvendo só
  `view`/`download`.
- [x] 8.12 **Web**: modal de destino navega níveis e escolhe a raiz; as três
  recusas produzem avisos distintos; ações alcançáveis em tela estreita
  (`viewport.ts` já existe para isso).
- [x] 8.13 `npm run lint`, `npm run build`, `npm run test` e `npm run format` na
